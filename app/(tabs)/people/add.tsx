...
export default function AddPersonScreen() {
  ...
  const savePerson = async () => {
    ...
    try {
      ...
      Alert.alert("Unable to save person", "Try again.");
      console.error(error);
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <TextInput
        placeholder="Ex: Daughter, Friend"
        placeholderTextColor="#94a3b8"
        value={relationship}
      ...
      <Text style={styles.helperText}>
        Add {MIN_PHOTOS} to {MAX_PHOTOS} photos for enrollment ({photoUris.length}/{MAX_PHOTOS}).
      </Text>
      ...
      <Pressable onPress={savePerson} style={[styles.saveButton, !canSave && styles.disabledButton]}>
        <Text style={styles.saveText}>{isSaving ? "Saving..." : "Save Person"}</Text>
      </Pressable>
    </ScrollView>
  );
}
